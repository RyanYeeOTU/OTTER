...
std::string windowTitle = "Ryan Yee - 100785620";
...
int main() {
	...
	static const GLfloat points[] = { // the triangle
		-1.f, -0.5f, 0.5f,
		0.5f, -0.5f, 0.5f,
		-1.f, 0.5f, 0.5f
	...
	Shader* shaderTwo = new Shader(); // square shader
	shaderTwo->LoadShaderPartFromFile("shaders/vertex_shader.glsl", ShaderPartType::Vertex);
	shaderTwo->LoadShaderPartFromFile("shaders/frag_shader2.glsl", ShaderPartType::Fragment); // second shader
	shaderTwo->Link();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shaderTwo->Bind();//bind shader duos
		vao2->Bind();
